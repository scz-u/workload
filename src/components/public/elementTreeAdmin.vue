...
export default {
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        if (Object.keys(data.query_result).length != 0 && Object.keys(data.query_result).indexOf('party') != -1) {
          for (let i in data.query_result.party.department_id) {
            datas.push({ id: data.query_result.party.department_id[i].toString(), UserId: data.query_result.party.department_id[i].toString(), IsDepart: true })
          }
          // this.datalist = datas
        }
        if (Object.keys(data.query_result).length != 0 && Object.keys(data.query_result).indexOf('user') != -1) {
          for (let i in data.query_result.user.userid) {
            data_user.push({ id: data.query_result.user.userid[i], IsDepart: false, UserId: data.query_result.usermap[data.query_result.user.userid[i]][0] + '-' + data.query_result.user.userid[i] })
          }
          // this.datalist = datas
          // } else {
          //   this.datalist = []
        ...
        for (var i = 0; i < datas.length; i++) {
          if (c.indexOf(this.datalist[i].toString()) > -1) {
            for (var j = 0; j < this.list_select.length; j++) {
              if (this.datalist[i].UserId == this.list_select[j].UserId) {
  ...
  mounted () {
    ...
    for (let i in this.departlist) {
      this.list_select.push({ UserId: this.departlist[i].toString(), IsDepart: this.departlist[i].split('-').length > 1 ? false : true })
    }
  },
  watch: {
    is_colse (val) {
    ...
    getdata () {
      ...
      console.log(this.departlist)
      for (let i in this.departlist) {
        this.selectlist.push(this.departlist[i].UserId)

  ...
  <div class="form_list_fill">
    <div v-if="!is_more">
      <!-- <div class="text_right top_form">
        <router-link :to="{ name: 'endFill', query: { k: Date.now() } }">
          <el-button icon="iconfont icon-cancel" size="small">返回</el-button>
        </router-link>
      </div> -->
      <div class="top_form">
        <span class="name_span">{{ name }} 任务填报表单</span>
        <span class="right">
          <router-link
            :to="{
              name: 'endFill',
...
export default {
  ...
  methods: {
    goClick (item) {
      this.$router.push({ name: 'endfilllistFill', query: { name: item.Title, id: beforeEncrypt(item.Code), taskname: this.$route.query.name, taskcode: this.$route.query.id } })
    },

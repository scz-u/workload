            ...
            <div>
              ...
              >
                <div class="item_list_img">
                  ...
                  <div class="item_title_fill_right right danger_title_fill">
                    关闭
                  </div>
...
export default {
  ...
  methods: {
    ...
    publicTaskClick (item) {
      this.$router.push({ name: 'publicFormFill', params: { other: 2 }, query: { id: beforeEncrypt(item.Code), name: item.Title } })
    },
    gourl (item) {
      this.$router.push({ name: 'endsumFill', query: { id: beforeEncrypt(item.Code), name: item.Title } })
    },
    editorTaskClick (item) {
      this.$router.push({ name: 'endformFill', query: { name: item.Title, id: beforeEncrypt(item.Code) } })
    },
    switchClick (items) {
      let $this = this
      let payload = {
    ...
    getalltaskbyuserid () {
      ...
      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];
      let posturl = $this.$apilist['getreportedtask'] + '?signature=' + keyvalue;
      // return false;
      // this.loading = true
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        console.log(data)
        ...
        if (data.data != null) {
          ...
          // console.log(this.task_type_list)
        } else {
          this.tasklist = {}
          this.task_type_list = []
        }

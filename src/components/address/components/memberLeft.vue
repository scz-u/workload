<template>
  <div class="memberLift address_left">
        ...
        <div @click="addressclick(node)">
          ...
          >
            <span class="el-dropdown-link">
              <i class="iconfont icon-depart-more"></i>
            </span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item :command="{ type: 'add', params: node }">
                <p>添加</p>
              </el-dropdown-item>
              <el-dropdown-item :command="{ type: 'editor', params: node }"
                ><p>修改</p>
              </el-dropdown-item>
              <el-dropdown-item :command="{ type: 'delete', params: node }">
                <p>删除</p>
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
    ...
    >
      <ul>
        <li class="option" @click.stop="addDepart">添加子部门</li>

        <li class="option" @click="editorDepart">修改名称</li>
        <li class="option" @click="delDepart">删除</li>
      </ul>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleCommand (command) {
      console.log(command)
      if (command.type == 'add') {
        this.addDepart(command.params)
      } else if (command.type == 'editor') {
        this.editorDepart(command.params)
      } else if (command.type == 'delete') {
        this.delDepart(command.params)
      }
    },
    searchClick () {
    ...
    delDepart (data) {
      ...
      this.$confirm('是否删除 ' + data.label + ' 部门?', '提示', {
        ...
        let payload = {
          corpid: this.$store.state.corpid,
          department_id: data.data.depart_id
        }
        ...
        let _this = this
        this.$server.post(posturl, createvalue['fd']).then(data1 => {
          console.log(data)
          // _this.departlist = data.data
          _this.getalldepart([data.data.ParentId])
          _this.$message({
            type: 'success',
            message: '删除部门成功！'
          });
    ...
    addDepart (data) {
      ...
      this.$prompt('部门名称', '新建部门', {
        ...
        this.$server.post(posturl, createvalue['fd']).then(data1 => {
          ...
          _this.getalldepart([data.data.ParentId, data.data.depart_id])
        }).catch(err => {
          console.log(err)
        })
    ...
    menuClick (node) {
      ...
      this.showlist = true
      this.clientX = e.clientX / 2 + 10 + 'px'

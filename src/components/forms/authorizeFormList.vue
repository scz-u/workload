<template>
  <div class="form_list">
    <div v-if="!is_more">
      <div class="top_form">
        <span class="name_span">被授权表单</span>
      </div>
      <div class="text_center" v-if="Object.keys(formlist).length == 0">
              ...
              <div class="list_box">
                ...
                <div>
                  <ul class="operation flex right">
                    <li style="width: 77px" class="text_center">
                      <span @click="gorFormClick(items.Code)"> 查看 </span>
                    </li>
                    <li>
                      <span @click="gorCopyClick(items.Code)">复制</span>
                    </li>
                  </ul>
                </div>
              </div>
...
<script>
import MoreFormList from './components/moreAuthorizeAlert'
...
export default {
  name: 'form_list',
  components: { TypeAlert, DepartAlert, MoreFormList },
  data () {
    return {
      active: [],
      num: [],
  ...
  methods: {
    ...
    getalltemplateform () {
      ...
      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];
      let posturl = $this.$apilist['getallauthorizetemplateform'] + '?signature=' + keyvalue;
      // return false;
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        console.log(data)

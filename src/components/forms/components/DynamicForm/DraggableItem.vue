...
const layouts = {
  colFormItem (h, element, index, parent, root) {
    ...
    let className = this.activeId === element.idx ? 'drawing-item active-from-item' : 'drawing-item'
    if (this.formConf.unFocusedComponentBorder) className += ' unfocus-bordered'

    if (element.tagIcon == 'divider') {
      return (
    ...
    } else if (element.tagIcon == 'describe') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { (activeItem(element), event.stopPropagation()) }}>
          <el-form-item label-width={element.labelWidth ? `${element.labelWidth}px` : null}
          >
            <el-tag>{element.placeholder}</el-tag>
    ...
    } else if (element.tagIcon == 'describeImg') {
      // this.getimgurl(element.placeholder)
      // console.log(element.placeholder)
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { (activeItem(element), event.stopPropagation()) }}>
          <el-form-item label-width={element.labelWidth ? `${element.labelWidth}px` : null} class="text_center"
          >
            <el-image src={(element.placeholder.split('说明图片')[0])}></el-image>
  ...
  rowFormItem (h, element, index, parent) {
    ...
    const className = this.activeId === element.idx ? 'drawing-row-item active-from-item' : 'drawing-row-item'
...
function renderChildren (h, element, index, parent) {
  ...
  return element.children.map((el, i) => {
    ...
    // const layout = layouts['colFormItem']
    if (layout) {
...
export default {
  ...
  methods: {
    getimgurl (key) {
      console.log('sdf')
      console.log(key.indexOf('https://xxx/dialog_icon.png'))
      if (key.indexOf('https://xxx/dialog_icon.png') == 0) {

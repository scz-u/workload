<template>
  <div class="task_index frame_right">
    <div v-show="!is_more">
      <div class="top_form">
        <span class="name_span">已完成任务</span>
        <span class="right">
...
import MoreTaskList from './components/moreCTaskList'
...
export default {
  ...
  methods: {
    ...
    goTaskGClick (item) {
      this.$router.push({ name: 'gatherTask', params: { other: 3 }, query: { id: beforeEncrypt(item.Code), name: item.Title } })
    },
    goTaskClick (item) {
    ...
    getalltaskbyuserid () {
      ...
      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];
      let posturl = $this.$apilist['getallclosedtaskbyuserid'] + '?signature=' + keyvalue;
      // return false;
      this.loading = true
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        console.log(data)

  ...
  'el-upload': {
    'tagIcon': (h, conf, key) => {
      ...
      if (conf['tagIcon'] === 'image') {
        ...
        list.push(<div slot="tip" class="el-upload__tip"><p class="upload_p">{conf.placeholder}</p></div>)
      } else {
        conf['list-type'] = 'text'
        list.push(<el-button size="small" type="primary" icon="el-icon-upload">点击上传附件</el-button>)
        list.push(<div slot="tip" class="el-upload__tip"><p class="upload_p">只能上传不超过 20MB 的文件</p><p class="upload_p">{conf.placeholder}</p></div>)
      }
      console.log(conf)
      console.log('----------------------------------')
      // if (conf.showTip) {
      //   list.push(<div slot="tip" class="el-upload__tip">只能上传不超过 20MB 的{conf.accept}文件</div>)
      // }
      return list
    },

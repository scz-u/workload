    ...
    >
      <el-form @submit.native.prevent label-width="100px">
        ...
        <el-form-item label="公示状态">
          <el-radio v-model="radio" :label="1">开始</el-radio>
          <el-radio v-model="radio" :label="0">结束</el-radio>
        </el-form-item>
        ...
        <el-form-item label="公示部门" v-if="this.radio == 1">
          ...
          >
            ...
            <!-- <div v-else>
              <span class="departid_span">选择部门</span>
            </div> -->
          </div>
        </el-form-item>
  ...
  methods: {
    ...
    getpubinfo () {
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        if (data.data.PubScope != null && data.data.PubScope != '') {
    ...
    updatepubinfo () {
      ...
      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];
      let posturl = $this.$apilist['updatepubinfo'] + '?signature=' + keyvalue;
      // return false;
      console.log(payload)
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        console.log(data)

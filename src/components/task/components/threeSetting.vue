    ...
    <div class="left_nav" v-if="is_nav">
      ...
      <div class="content_nav">
        <div class="quick-design-item" @click="switchClick(1)">
          <div>
            <div class="quick-design-item-name">
              <i class="iconfont icon-f1"></i>
              简单流程
            </div>
...
export default {
  ...
  components: { Simple, Subcondition, Conditions },
  data () {
    return {
      show_num: 0,
      is_nav: true,
  ...
  watch: {
    key (val) {
      ...
      if (this.mycode != '123') {
        ...
        this.worktype = this.sumval.threeval.workFlowDef.type
        if (this.sumval.threeval.workFlowDef.type == 1) {
          this.show_num = 1
        } else if (this.sumval.threeval.workFlowDef.type == 2) {
          this.show_num = 2
        } else {
          this.show_num = 3
        }

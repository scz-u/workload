      ...
      <el-row>
        ...
        <el-col :span="4" style="line-height: 60px">
          <el-button
            ...
            >保存并返回</el-button
          >
          <el-button size="small" @click="saveEditorClick()" v-else round
            >保存并返回</el-button
          >
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    saveSet (num) {
      ...
      console.log(this.val.threeval.nodeConfig)
      // return false;
      // if (num == 1) {
    ...
    gettaskfullinfo (code) {
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        console.log(data)
        console.log(data.data.base_info)
        console.log(data.data.base_info.departlist)
        this.val = {

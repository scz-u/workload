  ...
  methods: {
    ...
    calOther (val, indexs, assignval) {
      ...
      for (let i in assignval) {
        ...
        if (string(assignval[i].ToValue).split('')[assignval[i].ToValue.length - 1] == '.' || assignval[i].ToValue == '') break;
    ...
    detailbycode () {
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        this.getformdetail(data.data.FormCode)
        for (let i in this.form_fields) {

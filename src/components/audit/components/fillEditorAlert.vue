  ...
  <div class="fill_alert">
    ...
    >
      <div class="">
        <el-form
          label-position="left"
          :label-width="form_data.labelWidth"
          @submit.native.prevent
        >
          <el-form-item
            ><span class="font_weight_bold">{{ form_data.title }}表单</span>
          </el-form-item>
        ...
        >
          ...
          <div v-for="(item, indexS) in other_data.FField" :key="item">
            <el-form-item
              v-if="
            ...
            >
              <uploadFile :item="item" :fileList="item.value"></uploadFile>
            </el-form-item>
            <el-form-item
              ...
              :required="item.required"
            >
              <uploadImg :item="item" :imgList="item.value"></uploadImg>
            </el-form-item>
            <el-form-item
            ...
            >
              ...
              <div v-if="item.tagIcon == 'number'">
                ...
                <div v-if="item.assign_data != null">
                  ...
                  >
                    未分配值：{{
                      calOtherNum(item.value, indexS, item.assign_data)
                    }}
                  </span>
                  <div
                    v-for="(assignitem, index) in item.assign_data"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="close()" size="small" class="width_90" round
          >取 消</el-button
        >
        <el-button
          type="primary"
...
export default {
  ...
  methods: {
    ...
    calOtherNum (val, indexs, assignval) {
      console.log(indexs)
      ...
      for (let i in assignval) {
        console.log(typeof assignval[i].ToValue)
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        if (data.data.length > 0) {
          for (let i in data.data) {
            for (let j in this.other_data.FField) {
              if (this.other_data.FField[j].filed_code == data.data[i].field_code) {
                this.other_data.FField[j].value = data.data[i].value
              }
            }
          }
        }
        console.log(this.other_data.FField)
    ...
    submitClick () {
        ...
        if (this.form_fields[i].required) {
          if (this.form_fields[i].tagIcon == 'select_number') {
            ...
            if ((this.form_fields[i].value == null || this.form_fields[i].value == '') && this.form_fields[i].tagIcon != 'divider' && this.form_fields[i].tagIcon != 'describe' && this.form_fields[i].tagIcon != 'describeImg') {
              this.$alert('请填写 ' + this.form_fields[i].label + ' !', '提示', {
                confirmButtonText: '确定',
              });
              return false;
          ...
          if (this.form_fields[i].tagIcon == 'number') {
            ...
            formlist.push({ "field_code": this.form_fields[i].filed_code, "field_label": this.form_fields[i].label, "join_gather": this.form_fields[i].join_gather, "is_counted": this.form_fields[i].is_counted, "enable_greater": false, "enable_assign": this.form_fields[i].enable_assign, "value": this.getnowdate(this.form_fields[i].value) })
      ...
      let payload = {
        ...
        main_code: this.other_data.Code
      }

      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];

      let posturl = $this.$apilist['edit'] + '?signature=' + keyvalue;
      // return false;
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        console.log(data)
        this.$message({
    ...
    detailbycode () {
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        // this.userlist = data.data
        // this.userlist_two = Object.assign(data.data)
        this.other_data = data.data
        this.form_fields = this.other_data.FField
        for (let i in this.form_fields) {
          for (let j in this.other_data.FormFieldInfos) {
            if (this.form_fields[i].filed_code == this.other_data.FormFieldInfos[j].field_code) {

              this.form_fields[i].value = this.other_data.FormFieldInfos[j].value
              if (this.other_data.FormFieldInfos[j]['opt_label'] !== '') {
                this.form_fields[i].opt_label = this.other_data.FormFieldInfos[j].opt_label
                this.form_fields[i].is_counted = this.other_data.FormFieldInfos[j].opt_label
              }
            }
          }

          for (let m in this.detail.FormFieldInfos) {
            console.log('---------------------------')
            if (this.form_fields[i].filed_code == this.detail.FormFieldInfos[m].field_code && (this.detail.FormFieldInfos[m].opt_label == 'image' || this.detail.FormFieldInfos[m].opt_label == 'file')) {
              this.form_fields[i].value = this.detail.FormFieldInfos[m].value
            }
          }
        }
      }).catch(err => {
  ...
  watch: {
    is_add (val) {
      ...
      if (val) {
        console.log(this.detail)
        this.detailbycode()
        // this.form_fields = this.form_data.fields
      }
    },
    my_is_add (val) {
      this.$emit('back', val)

  ...
  methods: {
    backClick () {
      this.$confirm('未保存返回, 是否继续?', '提示', {
        ...
        this.$router.push({ name: 'IndexAdmin', query: { k: Date.now() } })
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消返回'
    ...
    saveSet (num) {
      ...
      let posturl = $this.$apilist['addtemplate'] + '?signature=' + keyvalue;
      // return false;
      console.log(payload)
    ...
    gettemplateformdetail () {
      ...
      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];
      let posturl = $this.$apilist['gettemplateformdetail_market'] + '?signature=' + keyvalue;
      // return false;
      console.log(payload)
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        console.log(data)
        ...
        for (let i in this.formData.fields) {
          if (this.formData.fields[i].tagIcon == 'describeImg') {
            let key = this.formData.fields[i].placeholder
            if (key.indexOf('https://xxx/dialog_icon.png') == -1) {
              ...
              $this.$server.post(posturl, createvalue['fd']).then((data) => {
                // console.log(data)
                // key = data.data
                this.formData.fields[i].placeholder = data.data
                // return data.data
              }).catch(err => {
                console.log(err)
              })

  ...
  methods: {
    ...
    submitForm1 (formName) {
      this.$refs[formName].validate((valid) => {
        console.log(valid)
        if (valid) {
          let payload = this.ruleFormP
          let _this = this

          let createvalue = this.$createObj(payload);
          var keyvalue = createvalue['keyvalue'];
          let posturl = this.$apilist['login_market'] + '?signature=' + keyvalue;
          console.log(payload)
          this.$server.post(posturl, createvalue['fd']).then(data => {
            console.log(data)
            if (data.data) {
              this.$store.dispatch('login_admin', { username: payload.userid, userid: payload.userid, }).then(() => {

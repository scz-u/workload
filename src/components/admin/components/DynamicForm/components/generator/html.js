...
//   return `<el-dialog  :append-to-body="true" v-bind="$attrs" v-on="$listeners" @open="onOpen" @close="onClose" title="Dialog Titile">
//     ${str}
//     <div slot="footer">
...
//       disabled, vModel, clearable, placeholder, width
//     } = attrBuilder(el)
//     const options = el.options ? `:options="field${el.idx}Options"` : ''
//     const props = el.props ? `:props="field${el.idx}Props"` : ''
...
//     const action = el.action ? `:action="field${el.idx}Action"` : ''
//     const multiple = el.multiple ? 'multiple' : ''
//     const listType = el['list-type'] !== 'text' ? `list-type="${el['list-type']}"` : ''
//     const accept = el.accept ? `accept="${el.accept}"` : ''
//     const name = el.name !== 'file' ? `name="${el.name}"` : ''
//     const autoUpload = el['auto-upload'] === false ? ':auto-upload="false"' : ''
//     const beforeUpload = `:before-upload="field${el.idx}BeforeUpload"`
//     const fileList = `:file-list="field${el.idx}fileList"`
...
//     children.push(`<el-option v-for="(item, index) in field${conf.idx}Options" :key="index" :label="item.label" :value="item.value" :disabled="item.disabled"></el-option>`)
//   }
//   return children.join('\n')
// }
...
//     children.push(`<${tag} v-for="(item, index) in field${conf.idx}Options" :key="index" :label="item.value" :disabled="item.disabled" ${border}>{{item.label}}</${tag}>`)
//   }
//   return children.join('\n')
// }

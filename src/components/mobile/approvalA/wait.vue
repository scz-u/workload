    ...
    <!-- <div class="footer_list" v-if="list.length > 0">
      ...
      <div class="right footer_btn">
        <cube-button :outline="true" :inline="true" @click="denysClick()"
          >拒绝</cube-button
        >
        <cube-button :primary="true" :inline="true" @click="applysClick()"
          >通过</cube-button
        >
      </div>
    </div> -->
...
export default {
  ...
  methods: {
    ...
    gourl (item) {
      ...
      setUser('detail_userid', beforeEncrypt(item.CreateUserId))
      // setUser('mission_state_one', beforeEncrypt(this.mission_state.toString()))
      // if (item.State == 1 || item.State == 4) {
      //   this.$router.push({ name: 'look_approval', query: { k: Date.now() } })
      // } else {
    ...
    alltoauditelist () {
      ...
      let createvalue = $this.$createObj(payload);
      var keyvalue = createvalue['keyvalue'];
      let posturl = $this.$apilist['alltoauditelist'] + '?signature=' + keyvalue;
      // return false;
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        console.log(data)

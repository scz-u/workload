<template>
  <div class="assign_list" v-if="my_is_add">
    <div v-if="!is_depart_m">
      ...
      <!-- {{ other_val }}{{ other_length }}{{ my_is_add }} -->
      <div class="middle_assign">
        ...
        <div class="editor_middle">
          ...
          <ul class="flex_ul" v-for="item in other_length" :key="item">
            <li>{{ other_val.left[item - 1].field_label }}</li>
            <li>{{ other_val.orgin[item - 1].value }}</li>
            <li>{{ other_val.used[item - 1].value }}</li>
            <li>{{ other_val.left[item - 1].value }}</li>
          </ul>
        </div>
                ...
                <div class="cube-validator cube-form-field">
                  <div class="cube-validator-content">
                    <div
                      ...
                      @click="departSelect()"
                    >
                      <getTreeNameM :openid="staging.to_userid"></getTreeNameM>
                    </div>
...
export default {
  name: 'assign_list',
  components: { DepartMAlert, getTreeNameM },
  props: {
    is_add: Boolean,
    detail: Object
  },
  ...
  methods: {
    changeNum (item) {
      console.log(item)
      console.log(this.other_val)
      let other = JSON.stringify(this.other_val1)
      for (let i in this.other_val1.left) {
        if (this.other_val1.left[i].filed_code == item.filed_code) {

          this.other_val1.used[i].value += parseFloat(item.value)
          this.other_val1.left[i].value -= parseFloat(item.value)
        }
      }
      this.other_val = JSON.parse(JSON.stringify(this.other_val1))
      this.other_val1 = JSON.parse(other)
    },
    delone (item) {
      let $this = this
      console.log(item)
    ...
    addAssignClick () {
      ...
      let common = JSON.parse(JSON.stringify(this.detail.FormFieldInfos))
      for (let i in common) {
        if (common[i].enable_assign) {
          this.staging.to_value.push({
            is_flag: false, rule_code: this.detail.RuleCode, filed_name: common[i].filed_name, filed_name_code: common[i].filed_name_code,
            filed_code: common[i].filed_code, filed_type: common[i].filed_type, value: '', is_count: true, enable_assign: common[i].enable_assign, enable_greater: common[i].enable_greater
          })
        }
      }
    ...
    leftvalue () {
      ...
      $this.$server.post(posturl, createvalue['fd']).then((data) => {
        ...
        this.other_val1 = JSON.parse(JSON.stringify(data.data))
        console.log(this.other_val)
        this.other_length = data.data.left.length
        this.staging = { data_code: this.detail.Code, to_userid: '', to_value: [] }
        let leftlist = []
        for (let i in data.data.left) {
          this.staging.to_value.push({ value: '', join_gather: data.data.left[i].join_gather, field_code: data.data.left[i].field_code, field_label: data.data.left[i].field_label, left: data.data.left[i].value, orgin: data.data.orgin[i].value, used: data.data.used[i].value })
        }
